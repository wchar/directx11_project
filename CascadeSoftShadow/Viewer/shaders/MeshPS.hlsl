...
cbuffer cbPerObject : register(b1)
{
    ...
    float3 g_vEmissive	: packoffset( c1 );
    float g_fStar		: packoffset( c1.w ); 

    float3 g_vSpecular	: packoffset( c2 );
    float g_fShininess	: packoffset( c2.w ); 
   
	float3 g_vAmbient	: packoffset( c3 ); 
    int g_iMaterialFlag	: packoffset( c3.w );
};
...
struct PS_OUTPUT
{
    float4 color        : SV_TARGET0;//color.rgba(xyzw)
    float4 normal       : SV_TARGET1;//normal.xyz(xyz),lighting(w)   
    float4 emissive     : SV_TARGET2;//emissive.RGB(xyz),shininess(w)
	float4 depth		: SV_TARGET3;//depth(x),depth^2(y),star(z),none(w)
};
...
{
    int bDiffuseTexture = g_iMaterialFlag % 2;
    int bEmissiveTexture = g_iMaterialFlag / 2 % 2;
    int bSpecular = g_iMaterialFlag / 4 % 2;
	...
	output.color = vDiffuse;
    output.normal = float4(input.normal, 1.0f);

    if (bEmissiveTexture)
    {
        output.emissive = g_texEmissive.Sample(g_samLinear, input.texcoord);
    }
    else
    {
        output.emissive = float4(1.0f, 1.0f, 1.0f, 1.0f);
    }
    output.emissive *= float4(g_vEmissive, 1.0f);
    output.emissive.w = g_fStar;

    if (bSpecular)
    {
        float3 vCamera = normalize(input.worldPos - g_vCameraPosition);
        float3 vReflection = reflect(g_vLightDir, input.normal);
        float  fPhongValue = saturate(dot(vReflection, vCamera));

        output.color.rgb += g_vSpecular * pow(fPhongValue, g_fShininess);
    }
	output.color += output.emissive;

	output.color *= fLighting;
	output.color = max(output.color, float4(g_vAmbient, 0.0f));
